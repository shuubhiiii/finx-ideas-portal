...
import { Users, Link2, UserPlus, UserCheck } from "lucide-react";
...
export default async function MembersPage() {
  ...
  const followersByUser = db.users.reduce<Record<string, number>>((acc, u) => {
        ...
        {members.map((m) => {
          ...
          return (
            <div key={m.id} className="card card-hover p-5">
              <div className="flex items-center gap-3">
                <Link href={`/portal/profile/${m.id}`} className={clsx("grid h-12 w-12 place-items-center rounded-full text-white text-sm font-semibold bg-gradient-to-br", avatarColor(m.id))}>
                  {initials(m.name)}
                </Link>
                <div className="min-w-0">
                  <Link href={`/portal/profile/${m.id}`} className="font-medium truncate hover:text-royal-700 block">
                    {m.name}
                  </Link>
              ...
              <div className="mt-4 flex items-center justify-between text-xs text-ink-muted">
                <span>
                  {ideasByUser[m.id] || 0} ideas · {followersByUser[m.id] || 0} followers
                </span>
                {m.link && (
              ...
              {!isMe && (
                <form action={`/api/users/${m.id}/follow`} method="POST" className="mt-3">
                  <button
                    type="submit"
                    className={clsx(
                      "btn-ghost w-full justify-center",
                      following && "text-royal-700 bg-royal-50",
                    )}
                  >
